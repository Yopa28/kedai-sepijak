<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">
          Kelola data pelayan Kedai Sepijak
        </p>
      </div>
    ...
    <div
      v-if="successMsg"
      class="rounded-lg border border-green-300 bg-green-50 p-3 text-green-700"
    >
      {{ successMsg }}
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
      <div class="overflow-x-auto">
        ...
        <!-- Empty -->
        <div
          v-if="!loading && filteredWaiters.length === 0"
        ...
        >
          ...
          <p class="mt-1 text-sm text-gray-500">
            Belum ada waiter atau tidak ditemukan dengan filter.
          </p>
        </div>
      </div>
      ...
      >
        <div class="flex items-center justify-between">
          <div class="text-sm text-gray-700">
            ...
            -
            <span class="font-medium">
              {{ Math.min(endIndex, filteredWaiters.length) }}
            </span>
            dari
            <span class="font-medium">
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900">
            {{ isEditing ? "Edit Waiter" : "Tambah Waiter" }}
          </h3>
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
          ...
          <div>
            ...
            <p class="text-xs text-gray-400 mt-1">
              Kode belum disimpan ke database karena backend Go belum
              mendukung field code.
            </p>
          </div>
          ...
          <div class="flex gap-3 pt-4">
            ...
            <button
              type="submit"
              :disabled="saving"
              class="flex-1 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 disabled:opacity-60"
            >
              {{
                saving
                  ? "Menyimpan..."
        ...
        <div class="flex items-start">
          <div class="flex-shrink-0">
            ...
            >
              <path
                ...
                stroke-linejoin="round"
                stroke-width="2"
                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3.034 1.732 3z"
              />
            </svg>
          </div>
          ...
          <div class="ml-3 flex-1">
            ...
            <p class="mt-2 text-sm text-gray-500">
              Yakin hapus
              <strong>{{ waiterToDelete?.name }}</strong>?
...
const filteredWaiters = computed(() => {
  let rows = Array.isArray(waiters.value)
    ? [...waiters.value]
    : [];
...
const totalPages = computed(() => {
  return Math.max(
    1,
    Math.ceil(
      filteredWaiters.value.length / perPage.value,
    ),
  );
});
...
const paginatedWaiters = computed(() => {
  return filteredWaiters.value.slice(
    startIndex.value,
    endIndex.value,
  );
});
...
async function reload() {
  ...
  try {
    const resp = await getAllWaiters({
      status:
        filters.value.status || undefined,
    });

    console.log("📦 Waiters response:", resp);
    ...
    if (Array.isArray(resp)) {
      ...
      rows = resp.data;
    } else if (Array.isArray(resp?.data?.waiters)) {
      rows = resp.data.waiters;
    } else if (Array.isArray(resp?.data?.data)) {
      rows = resp.data.data;
    }

    console.log("👥 Normalized waiters:", rows);
    ...
    console.error("❌ Gagal load waiter:", e);

    errorMsg.value =
      e?.response?.data?.message ||
      e?.message ||
      "Gagal memuat data waiter.";
...
async function submitForm() {
  ...
  try {
    ...
    console.log("📤 Waiter payload:", payload);
    ...
    if (isEditing.value && form.value.id) {
      ...
      successMsg.value =
        "Waiter berhasil diperbarui.";
    } else {
      await createWaiter(payload);

      successMsg.value =
        "Waiter berhasil ditambahkan.";
    }
...
async function toggleStatus(waiter) {
  errorMsg.value = "";
  successMsg.value = "";
  ...
  try {
    ...
    console.log(
      `🔄 Update waiter ${waiter.id} → ${newStatus}`,
    );
    ...
    successMsg.value =
      newStatus === "active"
        ? "Waiter berhasil diaktifkan."
        : "Waiter berhasil dinonaktifkan.";
...
async function deleteWaiter() {
  ...
  try {
    await apiDeleteWaiter(
      waiterToDelete.value.id,
    );

    closeDeleteConfirm();

    successMsg.value =
      "Waiter berhasil dihapus.";

    await reload();
  } catch (e) {
    console.error(
      "❌ Gagal menghapus waiter:",
      e,
    );

    deleteErr.value =
      e?.response?.data?.message ||
      "Tidak bisa menghapus waiter. Pastikan waiter tidak memiliki data terkait.";
  } finally {
